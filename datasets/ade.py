...
class ADESegmentation(data.Dataset):
    ...
    def __init__(self,
        ...
        sal_dir = os.path.join(self.root, 'sal', split)
        ...
        all_c = open(os.path.join('./datasets/data/ade/', 'all_cls.txt'), 'r')
        all_c = all_c.read().splitlines()
        self.all_c=[]
        for name in file_names:
            temp_cls=torch.zeros(150)
            for line in all_c:
                parts = line.split()
                if parts[0] == name:
        ...
        self.sal = [os.path.join(sal_dir, x + ".png") for x in file_names]
    ...
    def __getitem__(self, index):
        ...
        if self.image_set == 'train' or self.image_set == 'memory': #os.path.exists(self.sal_maps[index]):
            # sal_map = Image.open(self.sal[index])
            if os.path.exists(self.sal[index]):
                    
                sal_map = Image.open(self.sal[index])
            else:
                sal_map = np.where(~np.isin(target, self.target_cls) & (target != 0), 1, 0)
                sal_map = Image.fromarray(sal_map.astype(np.uint8))
        else:
